...
class OldSimulation : State {
    ...
    let disturbance = OldWaterDisturbance(columnCount: disturbanceCount)
    ...
    func reinit() {
        ...
        let horzWaterBuf: Int32 = (screenWidth - Int32(Self.waterColumnCount * OldWaterColumn.WIDTH)) / 2

        for i in 0..<Self.waterColumnCount {
            let column = OldWaterColumn(position: Vec2(
                x: Float64(horzWaterBuf) + Float64(OldWaterColumn.WIDTH * i), 
                y: OldWaterColumn.VERTICAL_ZERO))
            water.append(column)
        }
    ...
    func manageWater(isCrush: Bool, isSuspend: Bool) -> (String, Int32, Int32, Int32, Color) {
        ...
        var disturbances: [OldWaterColumn.Disturbance?] = Array(repeating: nil, count: water.count)
        do {
            var alterNum = 0
            var toPrint: [String] = []
    ...
    func update() {
        ...
        if Raylib.isKeyPressed(.space) {
            OldWaterColumn.waveCollisionsEnabled = !OldWaterColumn.waveCollisionsEnabled
        }
        ...
        Raylib.drawRectangle(verticalZeroBuf, Int32(OldWaterColumn.VERTICAL_ZERO), screenWidth - 2 * verticalZeroBuf, 5, Color.red)
